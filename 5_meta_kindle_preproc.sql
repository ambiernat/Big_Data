...
q4 as (
-- keep only those rows that do not contain 'minutes', 'kindle' or '<' strings as well as are not null
  select * 
  from q3
...
q2 as (
-- "explode" category column i.e. transform elements of the list in the category column into separate rows
-- this produces duplicates across the asin
  SELECT 
	asin,
	title,
	cat_list
